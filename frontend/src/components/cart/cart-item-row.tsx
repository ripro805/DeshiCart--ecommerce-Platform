...
export function CartItemRow({ item }: { item: CartItemType }) {
  ...
  const flatName = item.product_name ?? null;
  const flatImage = item.product_image ?? null;
  const qty = item.quantity || 1;
  const unitPrice =
    item.line_total != null && qty > 0
  ...
  return (
    <div className="flex gap-4 rounded-3xl border border-ink-200/60 bg-white/70 p-4 dark:border-ink-800/60 dark:bg-ink-950/60">
      <div className="relative h-24 w-24 shrink-0 overflow-hidden">
        <ProductImage src={flatImage} alt={flatName ?? "Product"} rounded="rounded-2xl" />
      </div>
      <div className="flex-1 min-w-0">
        <h3 className="truncate text-sm font-semibold">{flatName ?? "Product"}</h3>
        <Price value={unitPrice} size="sm" className="mt-1" />
        <div className="mt-3 flex items-center justify-between">
          <QtyStepper value={item.quantity} min={1} max={99} onChange={onQty} />
